...
/**
 * Bulk sends custom events to the Heap API servers.
 *
 * @param {Array} events to be sent to heap
 * @param {?function(Error)} callback called when the API call completes; error
 *   is undefined if the API call succeeds
 * @returns {Promise<boolean>} resolved with true if the API call completes
 *   successfully; rejected if the API call fails
 * @emits heap-api.Client#error
 * @see https://docs.heapanalytics.com/reference#bulk-track
 */
Client.prototype.bulkTrack = function(events, callback) {
  const options = {
    url: "https://heapanalytics.com/api/track",
    json: {
      app_id: this._validator.normalizeAppId(this.appId),
      events: events.map((event) => ({
        identity: this._validator.normalizeIdentity(event.identity),
        timestamp: event.timestamp,
        event: this._validator.normalizeEventName(event.event),
        properties: this._validator.normalizeProperties(event.properties),
      })),
    },
  };
